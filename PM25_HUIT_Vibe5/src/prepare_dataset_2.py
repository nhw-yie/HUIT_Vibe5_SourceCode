...
def load_station(path):
    ...
    df = df.rename(columns={'PM2.5': 'PM25','Temperature': 'T_obs','Humidity': 'RH_obs'})
    return df
...
def main():
    ...
    # Load dữ liệu
    station = load_station(data_dir / "station_2.csv")
    era5 = load_era5(data_dir / "era5_2.csv")
    modis = load_modis(data_dir / "modis_2.csv")
    ...
    # Save
    df.to_csv(out_dir / "dataset_features_2.csv", index=False)
    print("Saved final feature dataset to:", out_dir / "dataset_features_2.csv")
